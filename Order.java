...
public class Order implements Printable {
    ...
    private static final String orderFormat = "%-14s %d @ $%5.2f = $%6.2f\n";

    // Attributes

    private final Map<MenuItem, Integer> map = new HashMap<MenuItem, Integer>();
    ...
    public BigDecimal getSubTotal() {
    	...
    	for (Map.Entry<MenuItem, Integer> entry : map.entrySet()) {
    		subtotal = subtotal.add(new BigDecimal(entry.getValue()).multiply(entry.getKey().getPrice()));
    	}
	return subtotal;
    }
    ...
    public BigDecimal getTaxes() {
    	...
    	for (Map.Entry<MenuItem, Integer> entry : map.entrySet()) {
    		taxes = taxes.add(new BigDecimal(entry.getValue()).multiply(entry.getKey().getPrice()).multiply(TAX_RATE));
    	}
	return taxes;
    }
    ...
    public BigDecimal getTotal() {
    	BigDecimal price = this.getSubTotal().add(this.getTaxes());
    return price;
    }
    ...
    public String toString() {
    	String output = "";
    	for (Map.Entry<MenuItem, Integer> entry : map.entrySet()) {
    		BigDecimal total = entry.getKey().getPrice().multiply(BigDecimal.valueOf(entry.getValue()));
    		output += String.format(orderFormat, entry.getKey().getName(), entry.getValue(), entry.getKey().getPrice(), total);
    	}
    	output += String.format("\nSubtotal:                   $%6.2f\n", this.getSubTotal());
    	output += String.format("Taxes:                      $%6.2f\n", this.getTaxes());
    	output += String.format("Total:                      $%6.2f\n", this.getTotal());
	return output;
    }
